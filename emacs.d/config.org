    ...
    (setq interprogram-cut-function
          (lambda (text &optional push)
            (let ((process-connection-type nil))
              (let ((proc (start-process "win32yank" nil
                                       "win32yank.exe" "-i")))
              ...
              (with-temp-buffer
                (let ((proc (start-process "win32yank" nil
                  ...
                  (while (process-live-p proc)
                    (sleep-for 0.01))
                  (let ((clip (buffer-string)))
                    (unless (string= clip "")
                      ;; Remove Windows line endings (CRLF -> LF)
                      (setq clip (replace-regexp-in-string "\r\n" "\n" clip))
                      ;; Remove trailing newline if present
                      (if (string-suffix-p "\n" clip)
                          (substring clip 0 -1)
                        clip))))))))
  
  ;; Test function for WSL clipboard integration
  (defun test-wsl-clipboard ()
    "Test WSL clipboard integration by copying and pasting test text."
    (interactive)
    (let ((test-text "WSL clipboard test - this text should appear in Windows clipboard!"))
      (kill-new test-text)
      (message "Test text copied to clipboard: %s" test-text)
      (message "Try pasting in a Windows application, then copy some text from Windows and run M-x yank to test paste functionality.")))
...
* Crafterbin - File Upload Service
#+begin_src emacs-lisp
  (use-package crafterbin
    :load-path "/home/glenn/Projects/Code/crafterbin.el"
    :commands (crafterbin-upload crafterbin-upload-file crafterbin-upload-region))
#+end_src

* Consult-Line - Better Search with Vertico
#+begin_src emacs-lisp
  ;; Use consult-line for C-s - works perfectly with vertico + posframe
...
#+BEGIN_SRC sh :tangle no
  ...
  # rm ${ORG_DIR}/*.elc
  # This was a one-time fix for org-mode version 9 issues
...
Enable relative line numbers for programming and text modes.
#+begin_src emacs-lisp
;; Relative line numbers for specific modes
